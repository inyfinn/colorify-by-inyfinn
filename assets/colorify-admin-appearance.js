...
(function () {
	...
	function buildWpThemeTokens(tokens) {
		/* Akcent, NIE tło panelu — core/wtyczki malują tą zmienną tekst i przyciski. */
		var themeAccent = (tokens && tokens['--colorify-admin-accent']) || '#b4e717';
		return {
			'--wp-admin-theme-color': themeAccent,
			'--wp-admin-theme-color--rgb': hexToRgbCsv(themeAccent),
			'--wp-admin-theme-color-darker-10':
				'color-mix(in srgb, ' + themeAccent + ' 90%, #000)',
			'--wp-admin-theme-color-darker-20':
				'color-mix(in srgb, ' + themeAccent + ' 80%, #000)',
		};
	}
