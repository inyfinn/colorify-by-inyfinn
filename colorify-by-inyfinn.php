...
function colorify_enqueue_toolbar_assets( int $context_user_id = 0 ): void {
	...
	// Krytyczne pozycjonowanie paska (fallback gdy cache/encoding CSS).
	wp_add_inline_style(
		'colorify-admin-toolbar',
...
/**
 * GLOBALNY dark mode — wzorce strukturalne WP, inline-style overrides, CSS vars.
 * Ładowany po overrides, ale przed plugins — stanowi pomost.
 */
function colorify_enqueue_global_darkmode_css(): void {
	wp_enqueue_style(
		'colorify-global-darkmode',
		COLORIFY_PLUGIN_URL . 'assets/colorify-global-darkmode.css',
...
/**
 * CSS paneli wtyczek / motywów (VamTam, Elementor, …) — po global-darkmode.
 */
function colorify_enqueue_admin_plugins_css(): void {
	$deps = array( 'colorify-global-darkmode' );
	if ( wp_style_is( 'woocommerce_admin_styles', 'registered' ) ) {
		$deps[] = 'woocommerce_admin_styles';
	}

	wp_enqueue_style(
		'colorify-admin-plugins',
		COLORIFY_PLUGIN_URL . 'assets/colorify-admin-plugins.css',
		$deps,
 ...
 *
 * @param string $mce_css Comma-separated stylesheet URLs for TinyMCE.
 */
function colorify_filter_mce_dark_editor_css( string $mce_css ): string {
	if ( ! is_admin() || ! colorify_is_user_theme_enabled() ) {
		return $mce_css;
	}
	...
	$url = COLORIFY_PLUGIN_URL . 'assets/colorify-classic-editor-dark.css?ver=' . COLORIFY_PLUGIN_VERSION;
	return '' !== $mce_css ? $mce_css . ',' . $url : $url;
}
add_filter( 'mce_css', 'colorify_filter_mce_dark_editor_css' );
